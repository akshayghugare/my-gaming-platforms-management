...
import CreateCrmTag from '@/components/modals/crmTags/CreateCrmTag';
...
const buildDefaultForm = (category?: CrmTagCategory): CrmTagForm => ({
  id: '',
  name: '',
  description: '',
  category: category ?? '',
...
const CrmTagsTableList = ({ title, category }: Props) => {
  ...
  const [categoryFilter, setCategoryFilter] = useState<CrmTagCategory | ''>('');

  const [form, setForm] = useState<CrmTagForm>(buildDefaultForm(category));
  const [errors, setErrors] = useState<CrmTagErrors>({});
  ...
  const validate = (): CrmTagErrors => {
    const err: CrmTagErrors = {};
  ...
  const fetchTags = useCallback(async () => {
    try {
      ...
      console.error(e);
      toast.error('Failed to load CRM tags');
    } finally {
      setFetching(false);
    }
  ...
  const handleCreate = async () => {
    ...
    try {
      setLoading(true);
      const response = await apiService.post('/tags-crm/add', {
        name: form.name.trim(),
        description: form.description?.trim() || undefined,
        category: form.category,
      });
      if (response?.success) {
        toast.success(response?.message || 'CRM tag created successfully');
        closeCreateModal();
        if (page === 1) {
          fetchTags();
        } else {
      ...
      toast.error(apiErr.message || 'Failed to create CRM tag');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = (id: string): void => {
    DeleteRecord({
      endpoint: `/tags-crm/${id}`,
      successMessage: 'CRM tag deleted',
      onSuccess: fetchTags,
    });
  };
    ...
    <div className="px-4 w-full">
      <div className="flex justify-between items-center mb-6">
        <PageHeaderBreadcrumb
          title={title}
          items={[{ label: 'Home', clickable: true }, { label: 'CRM Tags' }]}
        />
      ...
      <div className="flex flex-wrap items-center gap-3 mb-5">
        ...
        {!category && (
          <select
            ...
            onChange={(e) => setCategoryFilter(e.target.value as CrmTagCategory | '')}
            className="px-4 py-2 rounded bg-slate-800 border border-slate-700 text-slate-200"
          >
            <option value="">All Categories</option>
            {CRM_CATEGORY_OPTIONS.map((opt) => (
      ...
      {showCreateModal && (
        <CreateCrmTag
          ...
          categoryOptions={CRM_CATEGORY_OPTIONS}
...
export default CrmTagsTableList;
