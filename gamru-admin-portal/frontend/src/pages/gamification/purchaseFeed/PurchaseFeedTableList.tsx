...
import { purchaseFeedColumns } from './columns';

const api = gamificationApi('purchase-feed');

const PurchaseFeedTableList: FC = () => {
  ...
  const fetchRows = useCallback(async () => {
    try {
      ...
      console.error(e);
      toast.error('Failed to load Purchase Feed');
    } finally {
      setFetching(false);
    }
  ...
  const handleDelete = (id: string) => {
    setMenuId(null);
    DeleteRecord({
      endpoint: `/gamification/purchase-feed/${id}`,
      successMessage: 'Purchase Feed Entry deleted',
      onSuccess: fetchRows,
    });
  };
      ...
      <div className="p-4 w-full">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex gap-3">
            <button
              onClick={() => navigate('/gamification/purchase-feed/archive')}
            ...
            <AdminOnly>
              <button
                onClick={() => navigate('/gamification/purchase-feed/create')}
                className="bg-blue-600 hover:bg-blue-700 px-4 py-2 rounded text-sm text-white"
              >
                Create Purchase Feed Entry
              </button>
            </AdminOnly>
          </div>
        </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {rows.length === 0 ? (
                <tr>
                  <td
                    colSpan={purchaseFeedColumns.length + 1}
                        ...
                        {menuId === row.id && (
                          <div className="absolute right-8 top-1 z-20 bg-slate-800 border border-slate-700 rounded shadow-lg min-w-[130px]">
                            <button
                              ...
                              onClick={() =>
                                navigate(`/gamification/purchase-feed/create?id=${row.id}`)
                              }
                            >
                              Edit
                            </button>
...
export default PurchaseFeedTableList;
