...
const columns = purchaseFeedColumns.filter((c) => c.header !== 'Status');

const PurchaseFeedArchive: FC = () => {
  ...
  const fetchRows = useCallback(async () => {
    try {
      ...
      console.error(e);
      toast.error('Failed to load archived Purchase Feed');
    } finally {
      setFetching(false);
    }
              ...
              {rows.length === 0 ? (
                <tr>
                  <td colSpan={columns.length + 1} className="p-10 text-center text-slate-400">
                    {fetching ? 'Loading…' : 'No archived Purchase Feed.'}
                  </td>
                </tr>
              ) : (
                rows.map((row) => (
...
export default PurchaseFeedArchive;
