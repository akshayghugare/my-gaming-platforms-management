...
import { ranksColumns } from './columns';

const api = gamificationApi('ranks');

const RanksTableList: FC = () => {
  ...
  const handleDelete = (id: string) => {
    setMenuId(null);
    DeleteRecord({
      endpoint: `/gamification/ranks/${id}`,
      ...
      <div className="p-4 w-full">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex gap-3">
            <button
              onClick={() => navigate('/gamification/ranks/archive')}
            ...
            <AdminOnly>
              <button
                onClick={() => navigate('/gamification/ranks/create')}
        ...
        <div className="overflow-x-auto border border-slate-700 rounded-md">
          <table className="w-full text-sm">
            ...
            <tbody>
              {rows.length === 0 ? (
                <tr>
                  <td colSpan={ranksColumns.length + 1} className="p-10 text-center text-slate-400">
                        ...
                        {menuId === row.id && (
                          <div className="absolute right-8 top-1 z-20 bg-slate-800 border border-slate-700 rounded shadow-lg min-w-[130px]">
                            <button
                              ...
                              onClick={() => navigate(`/gamification/ranks/create?id=${row.id}`)}
                            >
                              Edit
                            </button>
                            <button
