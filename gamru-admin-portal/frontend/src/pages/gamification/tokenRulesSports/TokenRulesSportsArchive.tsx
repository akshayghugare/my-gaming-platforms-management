...
const columns = tokenRulesSportsColumns.filter((c) => c.header !== 'Status');

const TokenRulesSportsArchive: FC = () => {
  ...
  const fetchRows = useCallback(async () => {
    try {
      ...
      console.error(e);
      toast.error('Failed to load archived Token Rules (Sports)');
    } finally {
      setFetching(false);
    }
      ...
      <div className="p-4 w-full">
        <div className="flex items-center justify-between mb-6">
          <PageHeaderBreadcrumb
            title="Token Rules (Sports) Archive"
            items={[
              ...
              {rows.length === 0 ? (
                <tr>
                  <td colSpan={columns.length + 1} className="p-10 text-center text-slate-400">
                    {fetching ? 'Loading…' : 'No archived Token Rules (Sports).'}
                  </td>
                </tr>
              ) : (
                rows.map((row) => (
...
export default TokenRulesSportsArchive;
