...
import { xpPointRulesSportsColumns } from './columns';
...
const XpPointRulesSportsTableList: FC = () => {
  ...
  const fetchRows = useCallback(async () => {
    try {
      ...
      console.error(e);
      toast.error('Failed to load XP Point Rules (Sports)');
    } finally {
      setFetching(false);
    }
  ...
  const handleDelete = (id: string) => {
    setMenuId(null);
    DeleteRecord({
      endpoint: `/gamification/xp-point-rules-sports/${id}`,
      ...
      <div className="p-4 w-full">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex gap-3">
            ...
            <button
              onClick={() => navigate('/gamification/xp-point-rules-sports/archive')}
            ...
            <AdminOnly>
              <button
                onClick={() => navigate('/gamification/xp-point-rules-sports/create')}
        ...
        <div className="overflow-x-auto border border-slate-700 rounded-md">
          <table className="w-full text-sm">
            <thead className="bg-slate-800">
              <tr>
                {xpPointRulesSportsColumns.map((c) => (
            ...
            <tbody>
              {rows.length === 0 ? (
                <tr>
                  <td
                    colSpan={xpPointRulesSportsColumns.length + 1}
                        ...
                        {menuId === row.id && (
                          <div className="absolute right-8 top-1 z-20 bg-slate-800 border border-slate-700 rounded shadow-lg min-w-[130px]">
                            <button
                              ...
                              onClick={() =>
                                navigate(`/gamification/xp-point-rules-sports/create?id=${row.id}`)
                              }
                            >
                              Edit
                            </button>
...
export default XpPointRulesSportsTableList;
