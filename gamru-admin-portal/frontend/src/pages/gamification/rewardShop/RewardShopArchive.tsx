...
const columns = rewardShopColumns.filter((c) => c.header !== 'Status');

const RewardShopArchive: FC = () => {
  ...
  const fetchRows = useCallback(async () => {
    try {
      ...
      console.error(e);
      toast.error('Failed to load archived Reward Shop');
    } finally {
      setFetching(false);
    }
    ...
    <DashboardLayout>
      <div className="p-4 w-full">
        <div className="flex items-center justify-between mb-6">
          ...
          />
          <button
            onClick={() => navigate('/gamification/reward-shop')}
              ...
              {rows.length === 0 ? (
                <tr>
                  <td colSpan={columns.length + 1} className="p-10 text-center text-slate-400">
                    {fetching ? 'Loading…' : 'No archived Reward Shop.'}
                  </td>
                </tr>
              ) : (
                rows.map((row) => (
...
export default RewardShopArchive;
