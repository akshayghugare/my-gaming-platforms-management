...
import { rewardShopColumns } from './columns';

const api = gamificationApi('reward-shop');

const extraFilters = ['Type', 'Ranks Eligibility', 'Supplier'];

const RewardShopTableList: FC = () => {
  ...
  const fetchRows = useCallback(async () => {
    try {
      ...
      console.error(e);
      toast.error('Failed to load Reward Shop');
    } finally {
      setFetching(false);
    }
  ...
  const handleDelete = (id: string) => {
    setMenuId(null);
    DeleteRecord({
      endpoint: `/gamification/reward-shop/${id}`,
      ...
      <div className="p-4 w-full">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex gap-3">
            <button
              onClick={() => navigate('/gamification/reward-shop/archive')}
            ...
            <AdminOnly>
              <button
                onClick={() => navigate('/gamification/reward-shop/create')}
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {rows.length === 0 ? (
                <tr>
                  <td
                    colSpan={rewardShopColumns.length + 1}
                        ...
                        {menuId === row.id && (
                          <div className="absolute right-8 top-1 z-20 bg-slate-800 border border-slate-700 rounded shadow-lg min-w-[130px]">
                            <button
                              ...
                              onClick={() =>
                                navigate(`/gamification/reward-shop/create?id=${row.id}`)
                              }
                            >
                              Edit
                            </button>
...
export default RewardShopTableList;
