...
import { xpPointRulesCasinoColumns } from './columns';

const api = gamificationApi('xp-point-rules-casino');
...
const extraFilters = ['Contribution Type', 'Game Category', 'Game provider', 'Game', 'Ranks'];

const XpPointRulesCasinoTableList: FC = () => {
  ...
  const fetchRows = useCallback(async () => {
    try {
      ...
      console.error(e);
      toast.error('Failed to load XP Point Rules (Casino)');
    } finally {
      setFetching(false);
    }
  ...
  const handleDelete = (id: string) => {
    setMenuId(null);
    DeleteRecord({
      endpoint: `/gamification/xp-point-rules-casino/${id}`,
      successMessage: 'XP Points Rule deleted',
      onSuccess: fetchRows,
    });
  };
      ...
      <div className="p-4 w-full">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex gap-3">
            ...
            <button
              onClick={() => navigate('/gamification/xp-point-rules-casino/archive')}
            ...
            <AdminOnly>
              <button
                onClick={() => navigate('/gamification/xp-point-rules-casino/create')}
                className="bg-blue-600 hover:bg-blue-700 px-4 py-2 rounded text-sm text-white"
              >
                Create XP Points Rule
              </button>
            </AdminOnly>
          </div>
        </div>
        ...
        <div className="overflow-x-auto border border-slate-700 rounded-md">
          <table className="w-full text-sm">
            <thead className="bg-slate-800">
              <tr>
                {xpPointRulesCasinoColumns.map((c) => (
            ...
            <tbody>
              {rows.length === 0 ? (
                <tr>
                  <td
                    colSpan={xpPointRulesCasinoColumns.length + 1}
                        ...
                        {menuId === row.id && (
                          <div className="absolute right-8 top-1 z-20 bg-slate-800 border border-slate-700 rounded shadow-lg min-w-[130px]">
                            <button
                              ...
                              onClick={() =>
                                navigate(`/gamification/xp-point-rules-casino/create?id=${row.id}`)
                              }
                            >
                              Edit
                            </button>
...
export default XpPointRulesCasinoTableList;
