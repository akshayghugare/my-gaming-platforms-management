...
const TokenRulesCasinoTableList: FC = () => {
  ...
  const fetchRows = useCallback(async () => {
    try {
      ...
      console.error(e);
      toast.error('Failed to load Token Rules (Casino)');
    } finally {
      setFetching(false);
    }
      ...
      <div className="p-4 w-full">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex gap-3">
            ...
            <button
              onClick={() => navigate('/gamification/token-rules-casino/archive')}
            ...
            <AdminOnly>
              <button
                onClick={() => navigate('/gamification/token-rules-casino/create')}
                className="bg-blue-600 hover:bg-blue-700 px-4 py-2 rounded text-sm text-white"
              >
                Create Token Rule Casino
              </button>
            </AdminOnly>
          </div>
        </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {rows.length === 0 ? (
                <tr>
                  <td
                    colSpan={tokenRulesCasinoColumns.length + 1}
                        ...
                        {menuId === row.id && (
                          <div className="absolute right-8 top-1 z-20 bg-slate-800 border border-slate-700 rounded shadow-lg min-w-[130px]">
                            <button
                              ...
                              onClick={() =>
                                navigate(`/gamification/token-rules-casino/create?id=${row.id}`)
                              }
                            >
                              Edit
                            </button>
...
export default TokenRulesCasinoTableList;
