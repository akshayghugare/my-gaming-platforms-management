...
import { tournamentsColumns } from './columns';
import TournamentLeaderboardModal from '@/components/modals/gamification/TournamentLeaderboardModal';

const api = gamificationApi('tournaments');

const TournamentsTableList: FC = () => {
  ...
  const [leaderboardFor, setLeaderboardFor] = useState<GamificationEntity | null>(null);
  ...
  const handleDelete = (id: string) => {
    setMenuId(null);
    DeleteRecord({
      endpoint: `/gamification/tournaments/${id}`,
    ...
    <DashboardLayout>
      <div className="p-4 w-full">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex gap-3">
            <button
              onClick={() => navigate('/gamification/tournaments/archive')}
            ...
            <AdminOnly>
              <button
                onClick={() => navigate('/gamification/tournaments/create')}
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {rows.length === 0 ? (
                <tr>
                  <td
                    colSpan={tournamentsColumns.length + 1}
                        ...
                        {menuId === row.id && (
                          <div className="absolute right-8 top-1 z-20 bg-slate-800 border border-slate-700 rounded shadow-lg min-w-[130px]">
                            <button
                              ...
                              onClick={() =>
                                navigate(`/gamification/tournaments/create?id=${row.id}`)
                              }
                            >
                              Edit
                            </button>
                            <button
                              className="w-full text-left px-3 py-2 text-sm text-blue-400 hover:bg-slate-700"
                              onClick={() => {
                                setMenuId(null);
                                setLeaderboardFor(row);
                              }}
                            >
                              Leaderboard
                            </button>
      ...
      {leaderboardFor && (
        <TournamentLeaderboardModal
          tournamentId={leaderboardFor.id}
          tournamentName={leaderboardFor.name}
          closeModal={() => setLeaderboardFor(null)}
        />
      )}
    </DashboardLayout>
  );
...
export default TournamentsTableList;
