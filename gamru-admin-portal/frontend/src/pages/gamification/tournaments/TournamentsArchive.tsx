...
const columns = tournamentsColumns.filter((c) => c.header !== 'Status');

const TournamentsArchive: FC = () => {
  ...
  const fetchRows = useCallback(async () => {
    try {
      ...
      console.error(e);
      toast.error('Failed to load archived Tournaments');
    } finally {
      setFetching(false);
    }
    ...
    <DashboardLayout>
      <div className="p-4 w-full">
        <div className="flex items-center justify-between mb-6">
          ...
          />
          <button
            onClick={() => navigate('/gamification/tournaments')}
              ...
              {rows.length === 0 ? (
                <tr>
                  <td colSpan={columns.length + 1} className="p-10 text-center text-slate-400">
                    {fetching ? 'Loading…' : 'No archived Tournaments.'}
                  </td>
                </tr>
              ) : (
                rows.map((row) => (
...
export default TournamentsArchive;
