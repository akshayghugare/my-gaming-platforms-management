...
import { missionsColumns } from './columns';
...
const api = gamificationApi('missions');

const MissionsTableList: FC = () => {
  ...
  const columns = [...missionsColumns, participatedCol((row) => setParticipantsFor(row))];
  ...
  const handleDelete = (id: string) => {
    setMenuId(null);
    DeleteRecord({
      endpoint: `/gamification/missions/${id}`,
      successMessage: 'Mission deleted',
      onSuccess: fetchRows,
    });
  };
      ...
      <div className="p-4 w-full">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex gap-3">
            <button
              onClick={() => navigate('/gamification/missions/archive')}
            ...
            <AdminOnly>
              <button
                onClick={() => navigate('/gamification/missions/create')}
                        ...
                        {menuId === row.id && (
                          <div className="absolute right-8 top-1 z-20 bg-slate-800 border border-slate-700 rounded shadow-lg min-w-[130px]">
                            <button
                              ...
                              onClick={() => navigate(`/gamification/missions/create?id=${row.id}`)}
                            >
                              Edit
                            </button>
                            <button
...
export default MissionsTableList;
