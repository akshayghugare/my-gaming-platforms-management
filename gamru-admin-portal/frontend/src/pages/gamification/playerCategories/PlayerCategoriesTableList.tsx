...
const PlayerCategoriesTableList: FC = () => {
  ...
  const fetchRows = useCallback(async () => {
    try {
      ...
      console.error(e);
      toast.error('Failed to load Player Categories');
    } finally {
      setFetching(false);
    }
  ...
  const handleDelete = (id: string) => {
    setMenuId(null);
    DeleteRecord({
      endpoint: `/gamification/player-categories/${id}`,
      successMessage: 'Player Category deleted',
      onSuccess: fetchRows,
    });
  };
      ...
      <div className="p-4 w-full">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex gap-3">
            <button
              onClick={() => navigate('/gamification/player-categories/archive')}
            ...
            <AdminOnly>
              <button
                onClick={() => navigate('/gamification/player-categories/create')}
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {rows.length === 0 ? (
                <tr>
                  <td
                    colSpan={playerCategoriesColumns.length + 1}
                        ...
                        {menuId === row.id && (
                          <div className="absolute right-8 top-1 z-20 bg-slate-800 border border-slate-700 rounded shadow-lg min-w-[130px]">
                            <button
                              ...
                              onClick={() =>
                                navigate(`/gamification/player-categories/create?id=${row.id}`)
                              }
                            >
                              Edit
                            </button>
...
export default PlayerCategoriesTableList;
