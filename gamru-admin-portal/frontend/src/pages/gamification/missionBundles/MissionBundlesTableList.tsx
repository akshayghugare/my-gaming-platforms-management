...
import { missionBundlesColumns } from './columns';
import { participatedCol } from '@/components/gamification/cells';
import ParticipantsModal from '@/components/modals/gamification/ParticipantsModal';

const api = gamificationApi('mission-bundles');

const MissionBundlesTableList: FC = () => {
  ...
  const [participantsFor, setParticipantsFor] = useState<GamificationEntity | null>(null);

  const columns = [...missionBundlesColumns, participatedCol((row) => setParticipantsFor(row))];
  ...
  const fetchRows = useCallback(async () => {
    try {
      ...
      console.error(e);
      toast.error('Failed to load Mission Bundles');
    } finally {
      setFetching(false);
    }
  ...
  const handleDelete = (id: string) => {
    setMenuId(null);
    DeleteRecord({
      endpoint: `/gamification/mission-bundles/${id}`,
      successMessage: 'Mission Bundle deleted',
      onSuccess: fetchRows,
    });
  };
    ...
    <DashboardLayout>
      <div className="p-4 w-full">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex gap-3">
            <button
              onClick={() => navigate('/gamification/mission-bundles/archive')}
            ...
            <AdminOnly>
              <button
                onClick={() => navigate('/gamification/mission-bundles/create')}
                        ...
                        {menuId === row.id && (
                          <div className="absolute right-8 top-1 z-20 bg-slate-800 border border-slate-700 rounded shadow-lg min-w-[130px]">
                            <button
                              ...
                              onClick={() =>
                                navigate(`/gamification/mission-bundles/create?id=${row.id}`)
                              }
                            >
                              Edit
                            </button>
      ...
      {participantsFor && (
        <ParticipantsModal
          ...
          entityId={participantsFor.id}
          entityName={participantsFor.name}
          closeModal={() => setParticipantsFor(null)}
        />
      )}
    </DashboardLayout>
  );
...
export default MissionBundlesTableList;
