...
const columns = missionBundlesColumns.filter((c) => c.header !== 'Status');

const MissionBundlesArchive: FC = () => {
  ...
  const fetchRows = useCallback(async () => {
    try {
      ...
      console.error(e);
      toast.error('Failed to load archived Mission Bundles');
    } finally {
      setFetching(false);
    }
              ...
              {rows.length === 0 ? (
                <tr>
                  <td colSpan={columns.length + 1} className="p-10 text-center text-slate-400">
                    {fetching ? 'Loading…' : 'No archived Mission Bundles.'}
                  </td>
                </tr>
              ) : (
                rows.map((row) => (
...
export default MissionBundlesArchive;
